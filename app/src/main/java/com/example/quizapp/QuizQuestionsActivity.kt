...
class QuizQuestionsActivity() : AppCompatActivity(), View.OnClickListener {
    ...
    constructor(parcel: Parcel) : this() {
        ...
        mSelectedOptionPosition = parcel.readInt()
    }
    ...
    private fun setQuestion() {
        ...
        if(mCurrentPosition == mQestionsList!!.size){
            btn_submit.text = "FINISH"
        }else{
            btn_submit.text = "SUBMIT"
        }

        val question = mQestionsList!!.get(mCurrentPosition - 1)
        ...
        when(v?.id){
            ...
            R.id.btn_submit ->{
                if(mSelectedOptionPosition == 0){
                    ...
                    when{
                        mCurrentPosition <= mQestionsList!!.size ->{
                            setQuestion()
                        }else ->{
                       val intent = Intent(this, ResultActivity::class.java)
                        intent.putExtra(Constants.USER_NAME, mUsername)
                        intent.putExtra(Constants.CORRECT_ANSWERS, mCorrectAnswers)
                        intent.putExtra(Constants.TOTAL_QUESTIONS, mQestionsList!!.size)
